...
이 모드에서는 서버를 재시작하면 검사·장바구니·주문 데이터가 초기화됩니다. 현재 Flutter의
AI 분석 버튼은 인증된 `/ai/analyze-routine` 경로에서 OpenAI 이미지 분석을 실행합니다.
기존 `compose`와 `suitability-analysis` 비동기 API 내부의 루틴 구성·점수 계산은 아직
화면 연동용 결정론적 결과이며, 다음 단계에서 같은 OpenAI 결과 정규화 로직으로 통합합니다.
